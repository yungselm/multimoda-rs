...
# Summary
Cardiac computed‑tomography angiography (CCTA) offers comprehensive 3D coronary anatomy but lacks the sub‑millimeter resolution and dynamic tissue detail available from intravascular ultrasound (IVUS) or optical coherence tomography (OCT). `multimodars` bridges this gap by registering high-resolution intravascular pullback frames to CCTA-derived centerlines and producing anatomically consistent, physiologically relevant 3D vessel models. Initially developed for the study of anomalous aortic origin of a coronary artery (AAOCA), the toolkit supports four gating paradigms (full, double-pair, single-pair and single) to register pullbacks acquired under different haemodynamic states, enabling analysis of pulsatile and stress-induced lumen deformation [@stark2025true], [@stark2025ivus]. Implemented as a Maturin project, `multimodars` exposes a Rust core—parallelised with Rayon—for performance-critical algorithms (Hausdorff-distance alignment, three-point rotational fitting, Hamiltonian ordering) with ergonomic PyO3 bindings that accept and return NumPy arrays. The project includes CI-driven tests, example notebooks and ReadTheDocs documentation to support reproducible workflows for researchers and clinicians developing patient-specific digital twins.
...
# Features
`multimodars` is a Rust-powered, Python-accessible toolkit for multi-modality intravascular/CCTA image fusion and registration that combines a high-performance native core with a NumPy-first I/O model for easy integration into research pipelines. Core algorithms (Hausdorff-based frame alignment, three-point rotational fitting, Hamiltonian ordering) are implemented in Rust and parallelized with Rayon, while ergonomic PyO3 bindings expose a compact, well-typed data model (`PyContourPoint`, `PyContour`, `PyGeometry`, `PyGeometryPair`) that maps to simple (N,4) NumPy arrays. The package supports four processings paradigms (full, double-pair, single-pair, single), round-trip conversion to/from CSV numpy arrays, and deterministic reconstruction of geometries from array inputs; utility methods provide centroiding, area/ellipticity metrics, smoothing, reordering, rotation/translation, and summary stenosis metrics.
