...
num_lights = 12
light_energy = (
    1000.0  # Blender 4's lights are more physically scaled; increase if too dim.
)
...
# -----------------------------
def setup_animation(
    groups, catheter_groups, total_frames, frame_rate, object_end_frame
):
    ...
    for frame in range(total_frames + 1):
        current_index = (
            frame % (2 * num_objects - 2) if (2 * num_objects - 2) > 0 else 0
        )
            ...
            try:
                if obj.animation_data and obj.animation_data.action:
                    for fcurve in obj.animation_data.action.fcurves:
                        if (
                            "hide_viewport" in fcurve.data_path
                            or "hide_render" in fcurve.data_path
                        ):
...
def setup_camera():
    ...
    max_dimension = (
        max(scene_dimensions.x, scene_dimensions.y, scene_dimensions.z)
        if max(scene_dimensions) != float("-inf")
        else 1.0
    )
    ...
    bpy.ops.object.camera_add(
        location=(
            scene_center.x,
            scene_center.y - max_dimension * 2.5,
            scene_center.z + max_dimension * 1.5,
        )
    )
...
if __name__ == "__main__":
    ...
    mesh_obj = next(
        (o for o in bpy.data.objects if o.name.startswith("lumen_000_full")), None
    )
    if mesh_obj:
