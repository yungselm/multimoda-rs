...
def to_array(generic) -> Union[np.ndarray, dict, Tuple[dict, dict]]:
    ...
    """
    from . import PyContour, PyCenterline, PyGeometry, PyGeometryPair, PyFrame, PyInputData
...
def _frame_to_numpy(frame) -> dict[str, np.ndarray]:
    ...
    result["lumen"] = np.array(lumen_pts, dtype=float) if lumen_pts else np.zeros((0, 4), dtype=float)
    
    # Add extras
    for contour_type, contour in frame.extras.items():
        pts = [(p.frame_index, p.x, p.y, p.z) for p in contour.points]
        result[contour_type.lower()] = np.array(pts, dtype=float) if pts else np.zeros((0, 4), dtype=float)
    
    # Add reference point
    if frame.reference_point:
        ref = frame.reference_point
        result["reference"] = np.array([[ref.frame_index, ref.x, ref.y, ref.z]], dtype=float)
    else:
...
def _input_data_to_numpy(input_data) -> dict[str, Union[np.ndarray, List[str], bool]]:
    ...
    if input_data.record:
        ...
        for record in input_data.record:
            ...
            row.append(record.measurement_1 if record.measurement_1 is not None else np.nan)
            row.append(record.measurement_2 if record.measurement_2 is not None else np.nan)
...
def numpy_to_geometry(
    ...
    def build_contour_from_array(arr: np.ndarray, frame_id: int, contour_type: str) -> PyContour:
        ...
        # Compute centroid
        centroid = (np.mean(pts_arr[:, 1]), np.mean(pts_arr[:, 2]), np.mean(pts_arr[:, 3]))
...
def geometry_to_frames_array(geometry: mm.PyGeometry) -> Dict[str, np.ndarray]:
    ...
    for frame in geometry.frames:
        ...
        # Add lumen
        lumen_pts = [(p.frame_index, p.x, p.y, p.z) for p in frame.lumen.points]
        frame_data["lumen"] = np.array(lumen_pts, dtype=float) if lumen_pts else np.zeros((0, 4), dtype=float)
        
        # Add extras
        for contour_type, contour in frame.extras.items():
            pts = [(p.frame_index, p.x, p.y, p.z) for p in contour.points]
            frame_data[contour_type.lower()] = np.array(pts, dtype=float) if pts else np.zeros((0, 4), dtype=float)
        
        # Add reference point
        if frame.reference_point:
            ref = frame.reference_point
            frame_data["reference"] = np.array([[ref.frame_index, ref.x, ref.y, ref.z]], dtype=float)
        else:
