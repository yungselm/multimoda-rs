...
pub fn double_pair_processing_rs(
    ...
    if geometries.len() != 4 {
        return Err(anyhow!("Double Pair processing requires exactly 4 geometries, got {}", geometries.len()));
    }
...
pub fn pair_processing_rs(
    ...
    let mut geometries = prepare_n_geometries(
        ...
        radius,
        n_points,
        Some(vec![input_data_a, input_data_b]
            .into_iter()
            .flatten()
            .collect()),
        input_path_a.map(Path::new),
    ...
    if geometries.len() != 2 {
        return Err(anyhow!("Single Pair processing requires exactly 2 geometries, got {}", geometries.len()));
    }

    let mut geom_a = geometries.remove(0);
    let mut geom_b = geometries.remove(0);

    let (mut geom_a, mut geom_b, logs_a, logs_b) = thread::scope(|s| -> Result<(
        Geometry,
        Geometry,
        Vec<AlignLog>,
        Vec<AlignLog>,
    ...
    let geom_pair = align_between_geometries(
        &mut geom_a,
        &mut geom_b,
        range_deg,
        step_deg,
...
pub fn single_processing_rs(
    ...
    if geom.len() != 1 {
        return Err(anyhow!("Single processing requires exactly 1 geometry, got {}", geom.len()));
    }
