    ...
    for step in 0..steps {
        ...
        for i in 0..n_frames {
                ...
                match contour_type {
                    ...
                    _ => {
                        if let (Some(start_contour), Some(end_contour)) = (
                            ...
                            let interp_contour = interpolate_contour(start_contour, end_contour, t)?;
            ...
            let reference_point = match (&start_frame.reference_point, &end_frame.reference_point) {
                (Some(start_rp), Some(end_rp)) => Some(interpolate_contour_point(start_rp, end_rp, t)),
...
fn interpolate_contour(start: &Contour, end: &Contour, t: f64) -> anyhow::Result<Contour> {
    ...
    Ok(Contour {
        ...
        pulmonary_thickness: interpolate_thickness(&start.pulmonary_thickness, &end.pulmonary_thickness, t),
        kind: start.kind,
    })
}
...
mod interpolation {
    ...
    #[test]
    fn test_interpolate_contours_with_reference_points() {
        let start = mock_geometry("start", 1);
        let end = mock_geometry("end", 1);
        let contour_types = &[ContourType::Lumen];
        ...
        // Should interpolate reference points
        let interp_frame = &result[1].frames[0];
        assert!(interp_frame.reference_point.is_some());
        
        let ref_point = interp_frame.reference_point.unwrap();
        assert_relative_eq!(ref_point.x, 0.0, epsilon = 1e-5);
        assert_relative_eq!(ref_point.y, 0.0, epsilon = 1e-5);
        assert_relative_eq!(ref_point.z, 0.0, epsilon = 1e-5);
    }
