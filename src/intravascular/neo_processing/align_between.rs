...
pub fn align_between_geometries(
    ...
    let best_rotation = find_best_rotation_between(
        &test_geom_a,
        &test_geom_b,
        step_rot_deg,
        rot_deg,
...
fn extract_geometry_points_with_frame_info(
    ...
    let total_points: usize = geometry.frames.iter()
        .map(|f| f.lumen.points.len())
        .sum();
...
fn find_best_rotation_between(
    ...
    let cost_fn = |angle: f64| -> f64 {
        ...
        let rotated_target: Vec<ContourPoint> = target
            ...
            .map(|point_info| {
                ...
                point_info.point.rotate_point(angle, (centroid.0, centroid.1))
            })
            .collect();

        let reference_points: Vec<ContourPoint> = reference
            .iter()
            .map(|pi| pi.point.clone())
            .collect();
    ...
    match step_deg {
        1.0..=f64::INFINITY => {
            search_range(cost_fn, step_deg, range_deg, None, range_deg)
        }
        0.1..1.0 => {
